...
**A1. I chose a grouped bar chart because it shows the comparison between the two candidate's percentages in each county very well.**

<https://datawrapper.dwcdn.net/vdh4b/2/>
...
md_gov_county <- md_gov_county |>
  ...
  mutate(total_votes_recent = Moore + Cox + Lashar + Wallace + Harding + Sefcik,
         Moore_percentage = (Moore/total_votes_recent)*100,
         total_votes_former = Hogan + Jealous + Quinn + Schlakman,
         Hogan_percentage = (Hogan/total_votes_former)*100) |>
  select(County, Moore_percentage, Hogan_percentage)
...
**A2. How the voting methods are also recovering from COVID is certainly a story in my mind. You can clearly see the effect that the**

<https://datawrapper.dwcdn.net/EHkuI/1/>
...
md_votes_by_type <- md_votes_by_type |>
  mutate(percentage_early = (Early/Total)*100,
         percentage_election_day = (`Election Day`/Total)*100,
         percentage_mail = (Mail/Total)*100) |>
  select(Year, percentage_early, percentage_election_day, percentage_mail)
...
**A3. The story could be found by looking at socioeconomic statuses of each area in moco and looking for a correlation between the overdose call numbers and those areas. Otherwise the story would just be saying which areas of Montgomery County produce the most overdose calls and possibly looking into some potential reasons for that.**

<https://datawrapper.dwcdn.net/i0sUX/1/>
...
montgomery_2022_overdoses <- montgomery_2022_overdoses |>
  ...
  mutate(percentage_calls_per_zip = (total_calls/659)*100)
