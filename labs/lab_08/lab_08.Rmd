...
pg_call_month <- pg_call |>
  ...
  mutate(percentage_per_month = (month_calls / 1397)*100)

pg_call_month |>
  ggplot() +
  geom_bar(aes(x=month, weight=percentage_per_month)) + 
  labs(
...
**A3. There is a weak negative correlation in the scatterplot that was printed out. The highest number of calls all come from places with low median incomes, but there are also many places with low median incomes that have a very low number of calls. On the other hand, zipcodes with the highest median incomes do tend to have lower calls, which supports a correlation there.**

```{r}
pg_calls_zipcode <- pg_call |>
  filter(zipcode != '0', zipcode != 'Q', zipcode != 'NA') |>
  group_by(zipcode) |>
  summarize(total_calls = n())

md_median_income_zip <- get_acs(geography = "zcta", variables = "B19013_001", year=2019, state="MD") |> rename(zipcode = GEOID, median_income = estimate)

joined_data <- pg_calls_zipcode |>
  left_join(md_median_income_zip, by = "zipcode") |>
  select(zipcode, total_calls, median_income, moe)

joined_data |>
  ggplot() + 
  geom_point(aes(x=median_income, y=total_calls)) + 
  labs(
    title = "Total 911 Calls Compared with Median Income per Zipcode",
    x = "Median Income",
    y = "Total Calls"
  )
```
