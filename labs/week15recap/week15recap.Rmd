...
md_childcare_inspections <- md_counties |>
  inner_join(childcare_inspections, by=c('GEOID' = 'fips'))
```

##### A1: **The 'NAMELSAD' row is the best to use because you do not want Baltimore to appear twice as it does in the other column for Baltimore City and Baltimore County.**
...
md_childcare_inspections <- md_childcare_inspections |>
  mutate(percentage_overdue = (overdue/total)*100)

md_childcare_inspections |>
  ggplot() +
  geom_bar(aes(x = reorder (NAMELSAD, percentage_overdue), weight = percentage_overdue)) + 
  coord_flip() +
  labs(
...
md_childcare_inspections <- md_childcare_inspections |>
  left_join(md_children_under_5, 'GEOID')
...
md_counties |>
  ggplot() +
  geom_sf(data = md_childcare_inspections, aes(fill=rate)) + 
  theme_minimal()
```

##### **A4**: The more northern parts tend to be more lacking
...
cor.test(md_childcare_inspections$median_income, md_childcare_inspections$rate)
```
...
##### A5: There is no correlation between these two things, the p-value is well above .05 making it not have any significance
