...
**A1. Brown only got more votes in two counties**
...
md_statewide_county <- read.csv("data/md_statewide_county.csv") |>
  ...
  mutate(total_governor = moore + cox + lashar + wallace + harding + sefcik + gov_writein, 
        total_attgen = peroutka + brown + ag_writein,
        percentage_moore = (moore/total_governor)*100,
        percentage_brown = (brown/total_attgen)*100,
        moore_brown_diff = percentage_moore - percentage_brown)
...
md_statewide_geometry <- md_counties |>
  inner_join(md_statewide_county, join_by(county, geometry))
