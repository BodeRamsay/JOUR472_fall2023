...
In the space below, write the data type for the "id" column and the "seats" column (it's in light gray, offset by \< \>). **Answer shr for id and dbl for seats**
...
In the space below, write the name of the title from the first row of the "name" column. **Answer Directed Research**
...
**Answer: First line is selecting the umd_courses dataset. The second line is taking all the terms and grouping the results by them. The third and fourth lines are adding up all the records in each term in the dataset.**
...
Compared to the previous codeblock, what has changed and how did that affect the result? **Answer an extra line was added, being the arrange method that took the results and arranged the count_classes in descending order**
...
**Answer Department was added to the group_by method, adding another column to the table. The arrange line makes term get sorted into ascending order, while department is in alphabetical.**
...
Compared to the previous codeblock, what has changed and how did that affect the result? **Answer The total seats has been added to the summarise part, meaning that it is counting all the total seats for each term in addition to counting all the records of each term**
...
Compared to the previous codeblock, what has changed and how did that affect the result? What do the top terms have in common? **Answer I am getting an error whenever I try to run this, though I believe that there would have been two extra columns being the mean and median of the seats in each class per term or it would have been the mean and median of all terms. On top of this, it would have arranged from most total seats to least total seats**
...
**Answer Again for some reason I am receiving an error trying to run this saying could not find function "arrange." This time it would have grouped everything by departments instead of terms and it would have included rows for the max/min_seats.**
...
Note: I could not access task 13 and 14 so I did not see any patterns that might have arisen in those tasks.

1.  Why do certain terms have such significantly less classes than others? Are they Summer or Winter terms?

2.  Is there a direct correspondence between number of students and number of seats?

3.  How does this information affect the universities process of determining how many required seats are necessary to accommodate their students for future years?
