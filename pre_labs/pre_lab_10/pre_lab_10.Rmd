...
**Task** Run the following code to load data. Describe what you see in the answer space below. What columns exist in this data? **Answer I see some descriptions of the data and the columns that it has are ObjectID, zipcode, zipname, and more**
...
**Task** Run the following code. Describe the output in the space below: what kind of information does it contain? **Answer It gives a ton of different information about Maryland zip codes, where they are (coordinates) and a few other numbers describing it**
...
**Task** Run the following code. Describe the output in the space below. **Answer This gives a map of all the different zip codes in Maryland which is much easier to see and digest as an audience member**
...
**Task** Run the following code. Describe the output in the space below. **Answer Its a bunch of information about all of the counties in the United States**
...
**Task** Run the following code. Describe the output in the space below. **Answer It is the same information as the last block of code we ran but just about counties from Maryland. This is because of the filter being used**
...
**Task** Run the following code. Describe the output in the space below. **Answer This shows a map that depicts all of the Maryland counties on a map of the state**
...
**Task** Run the following code. Describe the output in the space below. **Answer This combines the geographical data to make a map of Maryland that shows all of the zipcodes borders and county borders**
...
**Task** Run the following code. Describe the output in the space below. **Answer This data shows all the foreclosures in the month of March for each zip code**
...
**Task** Run the following code. Describe the output in the space below. **Answer This joins our original Maryland zipcodes data set with the foreclosure data from the month of March**
...
**Task** Run the following code. Describe the output in the space below. **Answer This creates a map with the data that shows which zipcodes have the most foreclosures**
...
**Task** Run the following code. Describe the output in the space below. Be sure to input your census api key. **Answer This just loads all of the libraries and loads my census API key**
...
library(janitor)
library(tidycensus)
census_api_key("62bb74987f9cf4fc6263c89b5eda3c0f6d0a551b", install = TRUE)
```
...
**Task** Run the following code. Describe the output in the space below. **Answer This shows the number of notices of intent to foreclose for each county in Maryland**
...
**Task** Run the following code. Describe the output in the space below. **Answer this loads the population per county in Maryland**
...
**Task** Run the following code. Describe the output in the space below. **Answer This loads the two data sets**
...
**Task** Run the following code. Describe the output in the space below. **Answer this cleans up the data to get what we are looking for, only the population of each county**
...
**Task** Run the following code. Describe the output in the space below. **Answer This is the joined data from the foreclosures with the data from the population**
...
**Task** Run the following code. Describe the output in the space below. **Answer This joins the notices by county and also adds a rate of notices per population in each county**
...
**Task** Run the following code. Describe the output in the space below. **Answer This creates a couple data sets that are used to help make the choropleth map that is created below depicting the rate of notices per each county. This accounts for any counties having less population and therefore having less notices than other big counties**
...
**Task** Run the following code. Describe the output in the space below. **Answer This changes the colors used to show the same data from above**
...
**Task** Run the following code. Describe the output in the space below. What regional patterns do you see? **Answer This prints out a map that chooses the color of counties based on a range. The southwestern side of Maryland tends to have a higher rate while northern Maryland tends to have lower rates**
