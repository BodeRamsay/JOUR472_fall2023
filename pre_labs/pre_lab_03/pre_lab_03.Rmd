...
**Answer: It says, "Warning: One or more parsing issues, call \`problems()\` on your data frame for details." When you follow the instructions it prints out some data, showing what was expected and what actually happened.**
...
**Answer: There is no parsing errors**
...
**Answer: Vendor name is of the character type, amount is of the double type, and date is of the character type**
...
**Task** Run the following codeblock to count the number of payments. Looking at the result, what jumps out at you as potentially problematic? **Answer: There are categories that are registered as N/A**
...
**Answer: I see all of the grants starting at the smallest going to the largest. It gives us info on the grantor and grantee, the zip code, year, amount, description, category, and date. This shows the outliers of the data on the small side**
...
**Task** Run the following codeblock to use the `clean_names()` function from janitor to standardize column names. How does the function change the headers? Answer below.

**Answer: It detects consistencies and inconsistencies in names. It misses some things but for the most part organizes the names to make them more organized and easy to use data-wise**
...
**Answer: 2 dupes are possible according to the final category in the data**
...
**Answer: A new column was created with all the zips only having five digits in that column, making it easier to compare the data.**
...
**Answer:** 21202, no it does not because the Maryland tourism board is based in Baltimore and that is where this zip code is.

```{r}
md_grants_loans |>
  filter(str_detect(Grantor, "Maryland Tourism Board")) |>
  group_by(`Zip Code`) |>
