...
**Answer Because it selected only the id and title, and filtered all other categories.**
...
**Answer This includes every single theatre class and every single class that has 15 or more seats, instead of restricting it to only theatre classes that have 15 or more seats**
...
**Answer: Instead of writing dates the American way, it switches to the way the rest of the world writes its dates.**
...
**Answer It only includes dates with the first day of the month, and it adds up the total amount of each month for the total_amount column**
...
**Answer It is adding new columns for the total votes and the percentage of votes Moore received**
...
**Answer It changed by arranging the pct_moore variable in descending order because of the arrange method.**
...
**Answer Its just as useful, the only thing that changed is it is in ascending order, showing where he was least voted for.**
...
**Answer I found str_to_lower(), which converts all the characters in a string to lowercase, and str_to_title(), which capitalizes the first character of each word**
...
**Answer Its abbreviating each state by detecting the address and it is going through each address and figuring out which one to abbreviate it to like a chain of if else statements.**
...
**Answer It does make some sense, it is showing which state had the highest total amount of money used on something, what that is I cannot figure out.**
...
1.  Could you find percentages of classes with more than 15 seats in the class?

2.  Could you find the percentage of people meeting certain criteria in the expenditures data?

3.  Could you make the way classes are listed more easier to understand using abbreviations?
